...
第三方向分身提问时，核心环节不是单纯生成，而是**围绕回答目标组织推理**。从第一性原理看，理论最优解永远是：带着当前问题，重新访问本体全部历史原文，再完整扫描一遍。但这个成本工程上不可接受，所以推理系统必须依赖灵魂锚点这层缓存。

我认为，**推理的本质是在盖然性约束下，从问题走向答案的一种寻路过程**。起点是当前问题及其上下文，终点不是唯一的一句标准答案，而是一个在当前证据约束下足够站得住脚的回答状态。这个终点可能是：直接回答、保守回答、承认边界，或者生成新的探针后暂不下判断。

因此，推理不一定只有一条路径可行。有些路径先补事实依据，有些路径先补关系边界，有些路径先确定表达口吻，也有些路径会在盖然性不足时直接转入保守表达。ReMi 的目标不是在任何问题上都给出绝对判断，而是在当前证据下，尽量找到一条能够把回答推进到更高盖然性的路径；当所有路径都仍然无法把盖然性推进到足够高时，就应该承认边界，而不是强行输出。

当前实现里，推理主链已经统一收敛到 `AvatarInferenceRuntime`。它的核心流程不是一个孤立的 recall loop，而是一次受约束的路径搜索：
...
graph TD
    ...
    C --> D[按路径召回已回答锚点]
    D --> E[评估当前盖然性是否足够]
    E -- 不足 --> F[改写 Query / 换路径 / 补充召回]
    F --> C
    E -- 足够或边界明确 --> G[整理缺失信息与回答边界]
    G --> H[生成最终回复]
```

每一轮循环：LLM 审视当前已召回的锚点，判断它们是否足以支撑当前路径继续前进。这里判断的不是抽象上的真假，而是当前证据是否已经把回答推进到足够高的盖然性。"充分"不只是话题相关——分身需要知道自己是谁、对方是谁、该用什么口吻说话、哪些事实已经有证据、哪些部分仍然缺信息，这些都要被判断清楚。如果不够，就改写 query、换角度、换路径、补充遗漏维度；直到足够、或者明确承认边界。
...
这里有一个第一性原理的推导：**理论最优解是带着当前问题，重新访问本体全部历史原文，再逐一判断和问题的相关性，搜索出最优的推理路径**。这一定能给出最充分的结果，但成本不可接受。所以我们才需要锚点缓存：用“预先回答一批未来高频问题”的方式，把一部分原本需要临场重扫的认知提前固化下来。工程上的目标，不是直接替代原文，而是用受盖然性约束的路径搜索去逼近“重扫原文”的效果。
