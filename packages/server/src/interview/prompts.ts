...
1. 每个锚点包含一个 question（锚定问题）和 answer（用户的回答）
  2. question 必须是脱离当前对话上下文也能成立的自解释问题，稳定可复用，明确写出边界信息，而不是宽泛槽位；当需要区分槽位、避免歧义或术语裸奔时，question 必须补足边界要素（语境范围 / 成立条件 / 术语语义）；但对已经天然自解释的简单事实问题，不要求为了满足格式去臆造范围/条件/术语语义；不能机械复述当前消息原话，不能退化成对当前一句话的机械转写；可以是事实问题，也可以是认知问题
 ...
 4. 优先提取用户明确说出的身份、事项、目标、事件、经历、关系、地点、时间线与动机
  ...
  6. 如果一条消息中有多个独立信息点，应尽量分别提取；事实锚点与认知锚点可以并存。definition + judgment、branching conditions（如“如果…；但如果…”、“如果信息不完整；但如果是高压力且独立开发”）必须按消息级别拆分，不要跨消息合并成一个 question。拆分边界：不拆分同义补充、修辞重复、纯背景铺垫、删去子句后主判断不变的内容；可拆但非必须的是围绕同一对象的并列细节，以及主锚点已补术语语义后仍有可独立召回的简洁释义；总原则是避免把不同边界压扁，但不是把一句话切得越碎越好
