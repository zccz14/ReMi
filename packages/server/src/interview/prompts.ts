...
1. 每个锚点包含一个 question（锚定问题）和 answer（用户的回答）
 ...
 4. 优先提取用户明确说出的身份、事项、目标、事件、经历、关系、地点、时间线与动机
  ...
  12. 涉及术语定义锚点时，必须按以下规则判断：required：出现显式定义句式，如“X 就是…”、“我说的 X 指的是…”、“这里的 X 不是……而是……”，并且必须额外生成 1 条术语定义锚点；若同条消息还有 judgment，必须拆成 definition anchor + judgment anchor；optional：存在解释性同位语或释义短语，且同一条消息内还有针对该术语对象的独立判断/偏好/用途表达；forbidden：无定义句式且无解释性同位语/释义短语
