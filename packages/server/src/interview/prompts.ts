...
1. 每个锚点包含一个 question（锚定问题）和 answer（用户的回答）
 ...
 5. 如果 question 出现项目名/术语名/缩写/专有概念，且当前消息提供了语义对象或解释性短语，主 question 不得只保留裸术语名，必须补足语义对象；否则按术语定义锚点规则处理
  ...
  9. question 优先承载语境范围、成立条件、讨论对象，以保证槽位自解释；只有纯临时、不可复用的时间细节不要机械写进 question
  10. 当条件/范围是区分槽位、保证自解释所必需时，可以进入 question
  11. question 不得包含“这个”“那个”“刚才提到的”等依赖上下文才能解析的指代
  12. 涉及术语定义锚点时，必须按以下规则判断：required：出现显式定义句式，如“X 就是…”、“我说的 X 指的是…”，并且必须额外生成 1 条术语定义锚点；若同条消息还有 judgment，必须拆成 definition anchor + judgment anchor；optional：存在解释性同位语或释义短语，且同一条消息内还有针对该术语对象的独立判断/偏好/用途表达；forbidden：无定义句式且无解释性同位语/释义短语
