...
3. 优先提取用户明确说出的身份、事项、目标、事件、经历、关系、地点、时间线与动机
4. 如果一条消息中有多个独立信息点，应尽量分别提取；事实锚点与认知锚点可以并存
5. 只跳过与已有锚点明显等价的重复内容；信息粒度不同、背景补充、状态更新都不算重复
6. 凡是锚定本体的人格、认知、偏好、经历与事实，question 必须使用“我”作为主语，不得使用“用户”
7. question 应锚定一个稳定信息槽位，不把短期时间词直接写入 question；时间、条件、背景优先写进 answer
8. 如果 prompt 中需要示例，只能使用完全脱敏、虚构、不可回溯到真实用户的数据
9. 如果没有新的可提取内容，不要输出任何 <anchor> 标签
