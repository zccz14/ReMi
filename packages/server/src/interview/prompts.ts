...
1. 每个锚点包含一个 question（锚定问题）和 answer（用户的回答）
  2. question 必须是脱离当前对话上下文也能成立的自解释问题，稳定可复用，明确写出边界信息，而不是宽泛槽位；每条值得提取的 question 至少显式包含一个边界要素（语境范围 / 成立条件 / 术语语义），如果三者都没有、只剩宽泛槽位名，就不合格；不能机械复述当前消息原话，不能退化成对当前一句话的机械转写；可以是事实问题，也可以是认知问题
