...
1. 每个锚点包含一个 question（锚定问题）和 answer（用户的回答）
 2. question 必须是脱离当前对话上下文也能成立的自解释问题，明确写出边界信息，而不是宽泛槽位；可以是事实问题，也可以是认知问题
 3. 当前用户消息为准；recentMessages 仅可用于解析代词/省略，不得新增当前消息未明确给出的事实、定义或判断，也不能依赖模型先验、常识脑补或外部信息
 4. 优先提取用户明确说出的身份、事项、目标、事件、经历、关系、地点、时间线与动机
 5. 如果 question 出现项目名/术语名/缩写/专有概念，且当前消息提供了语义对象或解释性短语，主 question 不得只保留裸术语名，必须补足语义对象；否则按术语定义锚点规则处理
 6. 如果一条消息中有多个独立信息点，应尽量分别提取；事实锚点与认知锚点可以并存。definition + judgment、branching conditions 必须按消息级别拆分，不要跨消息合并成一个 question
 7. 只跳过与已有锚点明显等价的重复内容；信息粒度不同、背景补充、状态更新都不算重复
 8. 凡是锚定本体的人格、认知、偏好、经历与事实，question 必须使用“我”作为主语，不得使用“用户”
 9. question 应锚定一个稳定信息槽位，不把短期时间词直接写入 question；时间、条件、背景优先写进 answer
 10. question 不得包含“这个”“那个”“刚才提到的”等依赖上下文才能解析的指代
 11. 涉及术语定义锚点时，必须按以下规则判断：required：出现显式定义句式，如“X 就是…”、“我说的 X 指的是…”；optional：存在解释性同位语或释义短语，且同一条消息内还有针对该术语对象的独立判断/偏好/用途表达；forbidden：无定义句式且无解释性同位语/释义短语
 12. answer 保持短而聚焦，不要用一个长 answer 覆盖多个条件分支、多个对象或多个术语解释；该拆就拆
 13. 如果 prompt 中需要示例，只能使用完全脱敏、虚构、不可回溯到真实用户的数据
 14. 仅当当前消息没有新的事实、定义或判断可独立提取时，不要输出任何 <anchor> 标签
