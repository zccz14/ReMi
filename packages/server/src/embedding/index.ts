...
export function searchSimilar(
  ...
  const rows = db
    .prepare(`SELECT id, distance FROM ${table} WHERE embedding MATCH ? ORDER BY distance LIMIT ?`)
    .all(JSON.stringify(query), topK) as { id: string; distance: number }[];
  return rows;
}

export function deleteEmbedding(db: Database.Database, table: VecTable, id: string): void {
  db.prepare(`DELETE FROM ${table} WHERE id = ?`).run(id);
}
